  ...
  return (
    <div className="p-6 max-w-5xl overflow-y-auto" style={{ height: '100%' }}>
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2"><Camera size={24} className="text-amber-500"/> Relatório Fotográfico</h1>
        ...
        <>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={gerarPDF} disabled={gerando || selecionadas.size === 0}
              className="flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white font-medium rounded-lg text-sm disabled:opacity-50">
          ...
          <div className="grid grid-cols-3 lg:grid-cols-4 gap-3">
            {fotos.map(f => {
              ...
              return (
                <div key={f.id} className={`relative border-2 rounded-xl overflow-hidden cursor-pointer transition-all ${selected ? 'border-amber-500 shadow-md' : 'border-slate-200 dark:border-slate-700'}`}
                  ...
                  {selected && <div className="absolute top-1.5 right-1.5 w-5 h-5 bg-amber-500 rounded-full flex items-center justify-center"><span className="text-white text-[10px] font-bold">✓</span></div>}
