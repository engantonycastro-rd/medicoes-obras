...
interface ObraCache { id: string; nome_obra: string; local_obra: string; contrato_nome: string; contrato_id?: string }
...
export function AppMobilePage() {
  ...
  async function loadCache() {
    ...
    if (navigator.onLine) {
      ...
      // Buscar gestores vinculados aos contratos
      try {
        const { data: cgData } = await supabase.from('contrato_gestores').select('contrato_id, gestor_id')
        const { data: perfisData } = await supabase.from('perfis').select('id, nome, role').eq('ativo', true)
        if (cgData && perfisData) {
          const gestorMap: Record<string, string[]> = {}
          cgData.forEach((cg: any) => {
            if (!gestorMap[cg.gestor_id]) gestorMap[cg.gestor_id] = []
            gestorMap[cg.gestor_id].push(cg.contrato_id)
          })
          const gs = Object.entries(gestorMap).map(([gId, cIds]) => ({
            id: gId,
            nome: perfisData.find((p: any) => p.id === gId)?.nome || 'Gestor',
            contrato_ids: cIds,
          }))
          setGestores(gs)
        }
    ...
    // Enriquecer com contrato_id se online
    if (navigator.onLine) {
      try {
        const obraIds = obrasC.map(o => o.id)
        if (obraIds.length > 0) {
          const { data: obrasDB } = await supabase.from('obras').select('id, contrato_id').in('id', obraIds)
          if (obrasDB) {
            obrasDB.forEach((odb: any) => {
              const cached = obrasC.find(o => o.id === odb.id)
              if (cached) (cached as any).contrato_id = odb.contrato_id
            })
          }
        }
      } catch {}
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col" style={{ maxWidth: '100vw', overflow: 'hidden' }}>
      ...
      <header className="bg-slate-900 text-white px-4 py-3 flex items-center gap-3 shrink-0 sticky top-0 z-30">
        ...
        <div className="w-8 h-8 bg-primary-500 rounded-lg flex items-center justify-center shrink-0">
          <span className="text-slate-900 font-bold text-xs">MO</span>
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold truncate">{tela === 'obras' ? 'MedObras' : obraSelecionada?.nome_obra}</p>
        ...
        <div className="flex-1 p-4 overflow-y-auto">
          ...
          {!cacheLoaded ? (
            ...
            <div className="space-y-3">
              <p className="text-sm font-bold text-slate-800 dark:text-white mb-2">Minhas Obras ({obras.length})</p>

              {/* Filtro por gestor */}
              {gestores.length > 0 && (
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 flex items-center gap-2">
                  <Users size={14} className="text-slate-400 shrink-0"/>
                  <select value={filtroGestor} onChange={e => setFiltroGestor(e.target.value)}
                    className="flex-1 border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg px-2 py-1.5 text-xs">
              ...
              {(filtroGestor === 'todos' ? obras : obras.filter(o => {
                const gestor = gestores.find(g => g.id === filtroGestor)
                if (!gestor) return true
                return gestor.contrato_ids.includes((o as any).contrato_id)
              })).map(obra => (
