  ...
  return (
    <div className="p-6 max-w-5xl overflow-y-auto" style={{ height: '100%' }}>
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2"><Briefcase size={24} className="text-primary-500"/> Subempreiteiros</h1>
        ...
        <div className="flex gap-2">
          ...
          {isAdmin && <button onClick={() => { setEditId(null); setFRazao(''); setFCnpj(''); setFNome(''); setFTel(''); setFEmail(''); setFEspec(''); setFObs(''); setShowForm(true) }}
            className="flex items-center gap-2 px-4 py-2 bg-primary-500 hover:bg-primary-600 text-white font-medium rounded-lg text-sm"><Plus size={14}/> Novo</button>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4"><p className="text-[10px] text-slate-400 uppercase font-semibold">Valor contratado</p><p className="text-lg font-bold text-primary-600">{fmt(subObras.reduce((s,o) => s+Number(o.valor_contratado),0))}</p></div>
      </div>
          ...
          return (
            <div key={s.id} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
              <div className="p-4 flex items-center gap-3 cursor-pointer" onClick={() => setExpandido(expandido === s.id ? null : s.id)}>
                <div className="w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center shrink-0"><Briefcase size={18} className="text-primary-600"/></div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  {expandido === s.id ? <ChevronUp size={14} className="text-primary-500"/> : <ChevronDown size={14} className="text-slate-400"/>}
                </div>
              </div>
              {expandido === s.id && (
                <div className="border-t border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 p-4 space-y-2">
                  ...
                  {obrasDoSub.length > 0 ? obrasDoSub.map(so => (
                    <div key={so.id} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-3 flex items-center justify-between">
                      ...
                      <div className="text-right text-[10px]"><p className="text-slate-600">Contratado: {fmt(Number(so.valor_contratado))}</p><p className="text-emerald-600">Medido: {fmt(Number(so.valor_medido))}</p><p className="text-primary-600">Pago: {fmt(Number(so.valor_pago))}</p></div>
                    </div>
