...
interface PerfilStore {
  ...
  perfis: Perfil[]       // lista para o admin
  loading: boolean
  error: string | null
...
export const usePerfilStore = create<PerfilStore>((set, get) => ({
  ...
  fetchPerfilAtual: async () => {
    ...
    if (error || !data) {
      // Perfil não existe ainda — cria como ENGENHEIRO inativo
  ...
  ativarUsuario: async (userId) => {
    ...
    set(s => ({
      perfis: s.perfis.map(p => p.id === userId ? { ...p, ativo: true } : p)
    }))
  },
  ...
  desativarUsuario: async (userId) => {
    ...
    if (error) { set({ error: error.message }); throw error }
    set(s => ({
      perfis: s.perfis.map(p => p.id === userId ? { ...p, ativo: false } : p)
    }))
  },
  ...
  alterarRole: async (userId, role) => {
    ...
    if (error) { set({ error: error.message }); throw error }
    set(s => ({
      perfis: s.perfis.map(p => p.id === userId ? { ...p, role } : p)
    }))
  },
