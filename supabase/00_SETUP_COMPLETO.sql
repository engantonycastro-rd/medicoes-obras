-- ═══════════════════════════════════════════════════════════════════════════════
-- MediObras — Setup Completo do Banco de Dados (Supabase)
-- Versão: 9.0 — Consolidado (Schema + Todas Migrations)
-- 
-- INSTRUÇÕES:
-- 1. Crie um projeto no Supabase (https://supabase.com)
-- 2. Copie a URL e a ANON KEY do projeto
-- 3. Configure em src/lib/supabase.ts
-- 4. Abra o SQL Editor do Supabase
-- 5. Cole TODO este arquivo e execute
-- 6. ⚠️ ALTERE o e-mail do admin na seção "INSERE ADMIN" abaixo
-- ═══════════════════════════════════════════════════════════════════════════════

-- PASSO 0: Limpa tudo que possa existir
...
DROP FUNCTION IF EXISTS is_team_visible(UUID)       CASCADE;
...
-- PERFIS (3 roles: ADMIN, GESTOR, ENGENHEIRO)
CREATE TABLE perfis (
  ...
  email       TEXT NOT NULL,
  nome        TEXT,
  role        TEXT NOT NULL CHECK (role IN ('ADMIN','GESTOR','ENGENHEIRO')) DEFAULT 'ENGENHEIRO',
...
CREATE INDEX idx_perfis_gestor ON perfis(gestor_id);
...
CREATE TABLE contratos (
  ...
  bdi_preco_unitario    NUMERIC(14,4),
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- ⚠️ INSERE ADMIN — ALTERE O E-MAIL ABAIXO PARA O SEU
-- O usuário PRECISA já ter se cadastrado (auth.users) antes de rodar este SQL
-- ═══════════════════════════════════════════════════════════════════════════════

INSERT INTO perfis (id, email, role, ativo, nome)
SELECT id, email, 'ADMIN', TRUE, 'Administrador'
FROM auth.users
WHERE email = 'setordeorcamentos@rdconstrutora.com'  -- ← ALTERE AQUI
ON CONFLICT (id) DO UPDATE SET role = 'ADMIN', ativo = TRUE;

DO $$
DECLARE v_count INT;
BEGIN
  SELECT COUNT(*) INTO v_count FROM perfis WHERE role = 'ADMIN' AND ativo = TRUE;
  IF v_count = 0 THEN
    RAISE NOTICE '⚠️ AVISO: Nenhum admin encontrado. Certifique-se que o e-mail está correto e que o usuário já se cadastrou.';
  ELSE
...
-- Perfis: RLS desativado (evita recursão infinita em is_admin())
...
-- Verifica se auth.uid() pode ver dados de owner_id (equipe)
...
-- CONTRATOS (visibilidade por equipe)
CREATE POLICY "contratos_select" ON contratos FOR SELECT USING (is_team_visible(user_id));
CREATE POLICY "contratos_insert" ON contratos FOR INSERT WITH CHECK (is_team_visible(auth.uid()));
CREATE POLICY "contratos_update" ON contratos FOR UPDATE USING (is_team_visible(user_id));
CREATE POLICY "contratos_delete" ON contratos FOR DELETE USING (is_admin() OR auth.uid() = user_id);

-- OBRAS (visibilidade por equipe)
CREATE POLICY "obras_select" ON obras FOR SELECT USING (is_team_visible(user_id));
CREATE POLICY "obras_insert" ON obras FOR INSERT WITH CHECK (is_team_visible(auth.uid()));
CREATE POLICY "obras_update" ON obras FOR UPDATE USING (is_team_visible(user_id));
CREATE POLICY "obras_delete" ON obras FOR DELETE USING (is_admin() OR auth.uid() = user_id);
...
CREATE POLICY "servicos_insert" ON servicos FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM obras o WHERE o.id = obra_id AND is_team_visible(o.user_id)));
CREATE POLICY "servicos_update" ON servicos FOR UPDATE
  USING (EXISTS (SELECT 1 FROM obras o WHERE o.id = obra_id AND is_team_visible(o.user_id)));
...
CREATE POLICY "linhas_select" ON linhas_memoria FOR SELECT
  USING (is_admin() OR EXISTS (
    SELECT 1 FROM medicoes m JOIN obras o ON o.id = m.obra_id WHERE m.id = medicao_id AND is_team_visible(o.user_id)
  ));
...
SELECT '✅ TABELAS:' as info, string_agg(table_name, ', ') as lista
FROM information_schema.tables
WHERE table_schema = 'public' AND table_type = 'BASE TABLE';

SELECT '✅ ADMIN:' as info, email, role, ativo FROM perfis WHERE role = 'ADMIN' LIMIT 1;
